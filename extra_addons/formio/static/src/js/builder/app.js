...
class App extends Component {
  ...
  createBuilder() {
    ...
    const builder = new Formio.FormBuilder(
      document.getElementById("formio_builder"),
      self.schema,
      self.options
    );
    ...
    builder.instance.on("change", function (res) {
      if (!res.hasOwnProperty("components")) {
        ...
        alert(
          "This Form Builder is readonly. It's state is either Current or Obsolete. Refresh the page again."
        );
        return;
      } else {
        console.log("[Forms] Saving Builder...");
        $.jsonRpc
          .request(self.saveUrl, "call", {
