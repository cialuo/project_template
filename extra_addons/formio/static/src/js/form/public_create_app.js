...
function app() {
  class App extends OdooFormioForm {
    initForm() {
      ...
      this.configUrl =
        "/formio/public/form/create/" + this.builderUuid + "/config";
      this.submissionUrl = false;
      this.submitUrl =
        "/formio/public/form/create/" + this.builderUuid + "/submit";
    }

    publicSubmitDoneUrl() {
      return (
        this.params.hasOwnProperty("public_submit_done_url") &&
        this.params.public_submit_done_url
      );
    }
      ...
      if (submission.state == "submitted") {
        if (this.publicSubmitDoneUrl()) {
          ...
          window.parent.postMessage({
            odooFormioMessage: "formioSubmitDone",
            params: params,
          });
        }
      }
...
async function start() {
  const templates = await owl.utils.loadFile(
    "/formio/static/src/js/form/public_create_app.xml"
  );
