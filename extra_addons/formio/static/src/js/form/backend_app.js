...
function app() {
  class App extends OdooFormioForm {
    ...
    portalSubmitDoneUrl() {
      return (
        this.params.hasOwnProperty("portal_submit_done_url") &&
        this.params.portal_submit_done_url
      );
    }
    ...
    submitDone(submission) {
      if (submission.state == "submitted") {
        if (
          this.urlParams.get("portal") === "true" &&
          this.portalSubmitDoneUrl()
        ) {
...
async function start() {
  const templates = await owl.utils.loadFile(
    "/formio/static/src/js/form/backend_app.xml"
  );
