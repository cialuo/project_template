...
odoo.define("report_pdf_preview.PreviewHandler", function (require) {
  "use strict";
  ...
  var PDFHandler = BaseHandler.extend({
    ...
    createHtml: function (url, mimetype, extension, title) {
      var result = $.Deferred();
      var viewerUrlTempalte = _.template(
        "/report_pdf_preview/static/lib/PDFjs/web/viewer.html?file=<%= url %>"
      );
      result.resolve(
        $(QWeb.render("ViewerJSFrame", { url: viewerUrlTempalte({ url }) }))
      );
      return result;
    },
  });
  ...
  var OpenOfficeHandler = BaseHandler.extend({
    ...
    createHtml: function (url, mimetype, extension, title) {
      var result = $.Deferred();
      var viewerUrlTempalte = _.template(
        "/report_pdf_preview/static/lib/ViewerJS/index.html#<%= url %>"
      );
      result.resolve(
        $(QWeb.render("ViewerJSFrame", { url: viewerUrlTempalte({ url }) }))
      );
