      ...
      if (this.mode === "readonly") {
        ...
        if (this.attrs.options.swipe_field) {
          if (this.recordData[this.attrs.options.swipe_field]) {
            ...
            var product_image_ids = this.recordData[
              this.attrs.options.swipe_field
            ].data;
            ...
            for (var i = 0; i < product_image_ids.length; i++) {
              ...
              var img = jQuery("<img/>", {
                ...
                src:
                  ...
                  width +
                  "&id=" +
                  JSON.stringify(product_image_ids[i].data.id) +
                  "&unique=" +
                  time +
                  "#",
              });
        ...
        // default work for product model
          ...
          if (this.recordData.product_template_image_ids) {
            ...
            var product_image_ids = this.recordData.product_template_image_ids
              .data;
            var time = new Date().getTime().toString();
            var width = this.nodeOptions.size
              ? this.nodeOptions.size[0]
        ...
        if (swiper)
          swiper.parent().click(function (e) {
            ...
            var source_id =
              self.model +
              "/" +
              JSON.stringify(self.res_id) +
              "/" +
              name_field +
              "?unique=" +
              field_utils.format
                .datetime(self.recordData.__last_update)
                .replace(/[^0-9]/g, "") +
              "#";
            var attachments = [
              {
            ...
            if (
              self.attrs.options.swipe_field &&
              self.recordData[self.attrs.options.swipe_field]
            ) {
              ...
              if (current_img.data("id") >= 0)
                source_id =
                  ...
                  related.model +
                  "&field=image_1920&id=" +
                  JSON.stringify(current_img.data("id")) +
                  "&unique=" +
                  time +
                  "#";

              product_image_ids =
                self.recordData[self.attrs.options.swipe_field].data;
              for (var i = 0; i < product_image_ids.length; i++) {
                attachments.push({
                  filename: i,
                  id:
            ...
            // if from product
              ...
              if (self.recordData.product_template_image_ids) {
                ...
                if (current_img.data("id") >= 0)
                  source_id =
                    "?model=product.image&field=image_1920&id=" +
                    JSON.stringify(current_img.data("id")) +
                    "&unique=" +
                    time +
                    "#";
                ...
                product_image_ids =
                  self.recordData.product_template_image_ids.data;
                for (var i = 0; i < product_image_ids.length; i++) {
                  attachments.push({
                    filename: i,
                    id:
