...
class ChooseDeliveryCarrier(models.TransientModel):
    ...
    height = fields.Integer('chiều cao(cm)', required=True, default=1)
    ...
    convert_volume = fields.Integer('Khối lượng quy đổi', required=True, default=0, help='Là khối lượng được tính dựa theo công thức (DxRxC/5000)')
    ...
    def onchange_weight(self):
        ...
        if self.carrier_id.service:
            ...
            if match_service:
                ...
                raise UserError(_('Phương thức vận chuyển này hiện chưa hỗ trợ cho địa điểm nhận hàng, Vui lòng chọn phương thức vận chuyển khác.'))
            self.order_id.write({
                'weight': self.weight,
                'length': self.length,
                'width': self.width,
