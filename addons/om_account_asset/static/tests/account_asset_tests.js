  ...
  QUnit.module("fields", {}, function () {
    ...
    QUnit.test("basic rendering", function (assert) {
      ...
      assert.strictEqual(
        form.$("thead th").text(),
        "",
        "toggler column should have no title"
      );

      // check the classnames
      assert.ok(
        form
          .$(".o_deprec_lines_toggler_cell:nth(0) button")
          .hasClass("o_is_posted"),
      ...
      assert.ok(
        form
          .$(".o_deprec_lines_toggler_cell:nth(1) button")
          .hasClass("o_is_posted"),
      ...
      assert.ok(
        !form
          .$(".o_deprec_lines_toggler_cell:nth(2) button")
          .hasClass("o_is_posted"),
      ...
      assert.ok(
        !form
          .$(".o_deprec_lines_toggler_cell:nth(3) button")
          .hasClass("o_is_posted"),
      ...
      // check the titles
      assert.strictEqual(
        form.$(".o_deprec_lines_toggler_cell:nth(0) button").attr("title"),
        "Posted",
        "first line toggler should have correct title"
      );
      assert.strictEqual(
        form.$(".o_deprec_lines_toggler_cell:nth(1) button").attr("title"),
        "Posted",
        "second line toggler should have correct title"
      );
      assert.strictEqual(
        form.$(".o_deprec_lines_toggler_cell:nth(2) button").attr("title"),
        "Accounting entries waiting for manual verification",
        "third line toggler should have correct title"
      );
      assert.strictEqual(
        form.$(".o_deprec_lines_toggler_cell:nth(3) button").attr("title"),
        "Unposted",
        "fourth line toggler should have correct title"
      );

      // check disabled property
      assert.ok(
        form.$(".o_deprec_lines_toggler_cell:nth(0) button").attr("disabled"),
        "first line toggle should be disabled"
      );
      assert.ok(
        form.$(".o_deprec_lines_toggler_cell:nth(1) button").attr("disabled"),
        "second line toggle should be disabled"
      );
      assert.ok(
        form.$(".o_deprec_lines_toggler_cell:nth(2) button").attr("disabled"),
        "third line toggle should be disabled"
      );
      assert.ok(
        !form.$(".o_deprec_lines_toggler_cell:nth(3) button").attr("disabled"),
        "fourth line toggle should not be disabled"
      );

      // check the visibility: the widget should always be visible, regardless its value
      assert.strictEqual(
        form.$(".o_deprec_lines_toggler:visible").length,
        4,
