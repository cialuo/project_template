  ...
  var GanttModel = BasicModel.extend({
    ...
    dateAdd: function (date, offset, unit) {
      ...
      if (Math.abs(result.diff(date, "hours")) >= 24) {
        var tzOffsetDiff =
          result.clone().local().utcOffset() - date.clone().local().utcOffset();
        if (tzOffsetDiff !== 0) {
    ...
    __reload: async function (handle, params) {
      ...
      if ("groupBy" in params) {
        if (params.groupBy && params.groupBy.length) {
          this.ganttData.groupedBy = this._filterDateInGroupedBy(
            params.groupBy
          );
    ...
    _fetchData: function () {
      ...
      var context = Object.assign({}, this.context, {
        group_by: this.ganttData.groupedBy,
      });
      ...
      if (this.ganttData.groupedBy.length) {
        groupsDef = this._rpc({
          ...
          orderBy: this.ganttData.groupedBy.map(function (f) {
            return { name: f };
          }),
      ...
      return this.dp
        .add(Promise.all([groupsDef, dataDef]))
        .then(function (results) {
          ...
          self.ganttData.records = self._parseServerData(
            searchReadResult.records
          );
      ...
      if (!groupedBy.length) {
        ...
        rows = Object.keys(currentLevelGroups).map(function (key) {
          ...
          var groupRecords = _.filter(params.records, function (record) {
            return _.isEqual(
              record[groupedByField],
              subGroups[0][groupedByField]
            );
          });
          ...
          var row = {
            name: self._getFieldFormattedValue(
              value,
              self.fields[groupedByField]
            ),
          ...
          if (isGroup) {
            ...
            row.rows.forEach(function (subRow) {
              ...
              row.childrenRowIds = row.childrenRowIds.concat(
                subRow.childrenRowIds || []
              );
            });
          }
    ...
    _filterDateInGroupedBy(groupedBy) {
      return groupedBy.filter((groupedByField) => {
        var fieldName = groupedByField.split(":")[0];
        return (
          fieldName in this.fields &&
          this.fields[fieldName].type.indexOf("date") === -1
        );
      });
    },
  });
