...
class MyController(http.Controller):
    ...
    def take_exam(self, exam_slug):
        ...
        exam = http.request.env['etq.exam'].sudo().search(
            [('slug', '=', exam_slug)])[0]

        token = uuid.uuid4().__str__()
        exam_result = http.request.env['etq.result'].sudo().create(
            {'exam_id': exam.id, 'token': token, 'state': 'incomplete', 'user_id': request.env.user.id})

        if exam.fill_mode == "random":

            questions = random.sample(
                exam.questions, exam.fill_mode_random_number)
    ...
    def exam_result(self, **kwargs):
        ...
        exam_result = request.env['etq.result'].search(
            [('token', '=', values['token'])])
            ...
            if exam_question.question_type == 'multi_choice':
                ...
                if result_question.question.num_correct > 1:
                    ...
                    for option in exam_question.question_options:
                        post_name = "question" + \
                            str(exam_question.id) + "option" + str(option.id)
                ...
                for option in exam_question.question_options_blank:
                    post_name = "question" + \
                        str(exam_question.id) + "option" + str(option_count)

                    if values[post_name] != option.answer:
                        question_result = False

                    request.env['etq.result.question.option'].sudo().create(
                        {'question_id': result_question.id, 'question_options_value': values[post_name]})
