  ...
  OrgChart.prototype = {
    ...
    init: function (opts) {
      ...
      if (
        ...
        !$chartContainer.find(".oc-export-btn").length
      ) {
    ...
    triggerInitEvent: function () {
      ...
      var mo = new MutationObserver(function (mutations) {
        mo.disconnect();
        initTime: for (var i = 0; i < mutations.length; i++) {
          for (var j = 0; j < mutations[i].addedNodes.length; j++) {
            if (mutations[i].addedNodes[j].classList.contains("orgchart")) {
              if (
                that.options.initCompleted &&
                typeof that.options.initCompleted === "function"
              ) {
    ...
    zoomEndHandler: function (e) {
      ...
      if (oc.$chart.data("pinching")) {
        ...
        var diff =
          oc.$chart.data("pinchDistEnd") - oc.$chart.data("pinchDistStart");
        if (diff > 0) {
    ...
    setChartScale: function ($chart, newScale) {
      ...
      if (lastTf === "none") {
        ...
        if (lastTf.indexOf("3d") === -1) {
          ...
          if (
            targetScale > opts.zoomoutLimit &&
            targetScale < opts.zoominLimit
          ) {
            $chart.css(
              "transform",
    ...
    buildJsonDS: function ($li) {
      ...
      var subObj = {
        ...
        relationship:
          ($li.parent().parent().is("li") ? "1" : "0") +
          ($li.siblings("li").length ? 1 : 0) +
          ($li.children("ul").length ? 1 : 0),
      };
      ...
      if (relation === "parent") {
        ...
        if ($target.length) {
          if (
            $target.is(".hidden") ||
            (!$target.is(".hidden") &&
              $target.closest(".nodes").is(".hidden")) ||
            (!$target.is(".hidden") &&
              $target.closest(".verticalNodes").is(".hidden"))
          ) {
        ...
        $target = isVerticalNode
          ? $node.parent().children("ul")
          : $node.closest("tr").siblings(":last");
        if ($target.length) {
          if (!$target.is(".hidden")) {
            return { exist: true, visible: true };
          }
        ...
        $target = isVerticalNode
          ? $node.closest("ul")
          : $node.closest("table").parent().siblings();
        if (
          $target.length &&
          (!isVerticalNode || $target.children("li").length > 1)
        ) {
          if (
            !$target.is(".hidden") &&
            !$target.parent().is(".hidden") &&
            (!isVerticalNode ||
              !$target.closest(".verticalNodes").is(".hidden"))
          ) {
            return { exist: true, visible: true };
        ...
        if ($target.length) {
          if (
            !(
              ($target.closest(".nodes").length &&
                $target.closest(".nodes").is(".hidden")) ||
              ($target.closest("table").parent().length &&
                $target.closest("table").parent().is(".hidden")) ||
              ($target.parent().is("li") &&
                ($target.closest("ul").is(".hidden") ||
                  $target.closest(".verticalNodes").is(".hidden")))
            )
          ) {
            return { exist: true, visible: true };
    ...
    showParent: function ($node) {
      ...
      $parent
        ...
        .one("transitionend", { node: $node }, this.showParentEnd.bind(this));
    },
    ...
    hideChildren: function ($node) {
      ...
      var $animatedNodes = $lowerLevel
        .last()
        .find(".node")
        .filter(this.isVisibleNode.bind(this));
      var isVerticalDesc = $lowerLevel.last().is(".verticalNodes")
        ? true
        : false;
      if (!isVerticalDesc) {
        $animatedNodes
    ...
    hideSiblingsEnd: function (event) {
      ...
      var $siblings = direction
        ? direction === "left"
          ? $nodeContainer.prevAll(":not(.hidden)")
          : $nodeContainer.nextAll(":not(.hidden)")
        : $nodeContainer.siblings();
      $nodeContainer
        .closest(".nodes")
        .prev()
        .children(":not(.hidden)")
        .slice(1, direction ? $siblings.length * 2 + 1 : -1)
        .addClass("hidden");
      event.data.animatedNodes.removeClass("sliding");
      $siblings
        .find(".node:gt(0)")
    ...
    hideSiblings: function ($node, direction) {
      ...
      var $lines = $animatedNodes
        .closest(".nodes")
        .prevAll(".lines")
        .css("visibility", "hidden");
      $animatedNodes.eq(0).one(
        "transitionend",
        {
          node: $node,
    ...
    showSiblingsEnd: function (event) {
      ...
      if (this.isInAction($node)) {
        ...
        $node
          .children(".topEdge")
          .removeClass("oci-chevron-up")
          .addClass("oci-chevron-down");
      }
    },
    //
    ...
    showSiblings: function ($node, direction) {
      ...
      $visibleNodes
        ...
        .one(
          "transitionend",
          { node: $node, visibleNodes: $visibleNodes },
    ...
    startLoading: function ($edge) {
      ...
      if (
        typeof $chart.data("inAjax") !== "undefined" &&
      ...
      $edge
        .parent()
        .append('<i class="oci oci-spinner spinner"></i>')
        .children()
        .not(".spinner")
        .css("opacity", 0.2);
      $chart.data("inAjax", true);
    ...
    isInAction: function ($node) {
      return $node.children(".edge").attr("class").indexOf("oci-") > -1
        ? true
        : false;
    },
    //
    ...
    switchHorizontalArrow: function ($node) {
      ...
      if (
        ...
        (typeof opts.ajaxURL === "undefined" ||
          $node.closest(".nodes").data("siblingsLoaded"))
      ) {
        ...
        if ($prevSib.length) {
          if ($prevSib.is(".hidden")) {
            $node
              .children(".leftEdge")
              .addClass("oci-chevron-left")
              .removeClass("oci-chevron-right");
          } else {
            $node
              .children(".leftEdge")
              .addClass("oci-chevron-right")
              .removeClass("oci-chevron-left");
          }
        }
        ...
        $node
          .children(".leftEdge")
          .toggleClass("oci-chevron-right", sibsVisible)
          .toggleClass("oci-chevron-left", !sibsVisible);
        $node
          .children(".rightEdge")
          .toggleClass("oci-chevron-left", sibsVisible)
          .toggleClass("oci-chevron-right", !sibsVisible);
    ...
    nodeEnterLeaveHandler: function (event) {
      ...
      if (event.type === "mouseenter") {
        if ($topEdge.length) {
          ...
          $topEdge
            .toggleClass("oci-chevron-up", !flag)
            .toggleClass("oci-chevron-down", flag);
        }
        ...
        $node
          .children(".edge")
          .removeClass(
            "oci-chevron-up oci-chevron-down oci-chevron-right oci-chevron-left"
          );
      }
    },
    //
        ...
        .done(function (data) {
          if (that.$chart.data("inAjax")) {
            if (rel === "parent") {
              ...
              that.addSiblings(
                $edge.parent(),
                data.siblings ? data.siblings : data
              );
            }
          }
        })
    ...
    topEdgeClickHandler: function (event) {
      ...
      if (parentState.exist) {
        ...
        if (this.startLoading($topEdge)) {
          var opts = this.options;
          var url = $.isFunction(opts.ajaxURL.parent)
            ? opts.ajaxURL.parent($node.data("nodeData"))
            : opts.ajaxURL.parent + $node[0].id;
      ...
      if (siblingsState.exist) {
        ...
        if (this.startLoading($hEdge)) {
          ...
          var url = this.getNodeState($node, "parent").exist
            ? $.isFunction(opts.ajaxURL.siblings)
              ? opts.ajaxURL.siblings($node.data("nodeData"))
              : opts.ajaxURL.siblings + nodeId
            : $.isFunction(opts.ajaxURL.families)
            ? opts.ajaxURL.families($node.data("nodeData"))
    ...
    toggleVNodes: function (event) {
      ...
      if ($descendants.eq(0).is(".slide-up")) {
        ...
        $children
          ...
          .removeClass("slide-up")
          .eq(0)
          .one("transitionend", { vNodes: $children }, this.expandVNodesEnd);
      } else {
        $descendants
          ...
          .one(
            "transitionend",
            { vNodes: $descendants },
            this.collapseVNodesEnd
          );
        $descendants
          .find(".toggleBtn")
          .removeClass("oci-minus-square")
          .addClass("oci-plus-square");
      }
    },
    //
    createGhostNode: function (event) {
      ...
      ghostNode.setAttribute(
        "width",
        isHorizontal ? $nodeDiv.outerWidth(false) : $nodeDiv.outerHeight(false)
      );
      ghostNode.setAttribute(
        "height",
        isHorizontal ? $nodeDiv.outerHeight(false) : $nodeDiv.outerWidth(false)
      );
    ...
    filterAllowedDropNodes: function ($dragged) {
      ...
      var $dragZone = $dragged
        .closest(".nodes")
        .siblings()
        .eq(0)
        .find(".node:first"); // parent node
    ...
    dropHandler: function (event) {
      ...
      if (!$dropZone.closest("tr").siblings().length) {
        ...
        $dropZone
          .append('<i class="edge verticalEdge bottomEdge oci"></i>')
          .parent()
          .attr("colspan", 2)
          .parent()
          .after(
        ...
        var horizontalEdges =
          '<i class="edge horizontalEdge rightEdge oci"></i><i class="edge horizontalEdge leftEdge oci"></i>';
        $dropZone
          .closest("tr")
          .next()
          .addBack()
          .children()
          .attr("colspan", dropColspan);
        ...
        $dropZone
          ...
          .end()
          .next()
          .append($dragged.closest("table").parent());
        var $dropSibs = $dragged
          .closest("table")
          .parent()
          .siblings()
          .find(".node:first");
    ...
    touchmoveHandler: function (event) {
      ...
      if (!this.touchMoved) {
        ...
        this.touchDragImage = this.createDragImage(
          event,
          this.$chart.data("dragged")[0]
        );
      }
    ...
    touchendHandler: function (event) {
      ...
      if (this.touchMoved) {
        ...
        fakeMouseClickEvent.initMouseEvent(
          "click",
          true,
          true,
          window,
    ...
    createDragImage: function (event, source) {
      ...
      this.touchDragImageOffset = {
        x: sourcePoint.x - sourceRectangle.left,
        y: sourcePoint.y - sourceRectangle.top,
      };
    ...
    moveDragImage: function (event, image) {
      ...
      requestAnimationFrame(function () {
        ...
        if (orgChartMaster.touchDragImageOffset) {
          s.left =
            Math.round(pt.x - orgChartMaster.touchDragImageOffset.x) + "px";
          s.top =
            Math.round(pt.y - orgChartMaster.touchDragImageOffset.y) + "px";
        }
      });
    },
    //
    ...
    createNode: function (data) {
      ...
      if (opts.nodeTemplate) {
        ...
        $nodeDiv
          .append('<div class="title">' + data[opts.nodeTitle] + "</div>")
          .append(
      ...
      if (opts.verticalLevel && level >= opts.verticalLevel) {
        if (level + 1 > opts.verticalLevel && Number(flags.substr(2, 1))) {
          ...
          $nodeDiv.append(
            '<i class="toggleBtn oci oci-' + icon + '-square"></i>'
          );
        }
      } else {
        ...
        if (Number(flags.substr(2, 1))) {
          $nodeDiv
            ...
            .children(".title")
            .prepend(
              '<i class="oci ' + opts.parentNodeSymbol + ' symbol"></i>'
            );
        }
      }
      ...
      $nodeDiv.on(
        ...
        this.nodeEnterLeaveHandler.bind(this)
      );
      ...
      $nodeDiv.on(
        "click",
        ".bottomEdge",
        this.bottomEdgeClickHandler.bind(this)
      );
      $nodeDiv.on(
        "click",
        ".leftEdge, .rightEdge",
    ...
    buildHierarchy: function ($appendTo, data) {
      ...
      if (data.level) {
        ...
        level = data.level =
          $appendTo.parentsUntil(".orgchart", ".nodes").length + 1;
      }
      // Construct the node
      ...
      if (hasChildren) {
        var isHidden =
          level + 1 > opts.visibleLevel || data.collapsed ? " hidden" : "";
        var isVerticalLayer =
          opts.verticalLevel && level + 1 >= opts.verticalLevel ? true : false;
        var $nodesLayer;
        if (isVerticalLayer) {
          $nodesLayer = $("<ul>");
          ...
          if (level + 1 === opts.verticalLevel) {
            $appendTo
              .children("table")
              .append(
                '<tr class="verticalNodes' + isHidden + '"><td></td></tr>'
              )
              .find(".verticalNodes")
              .children()
              .append($nodesLayer);
          ...
          var lowerLines =
            '<tr class="lines' + isHidden + '"><td class="rightLine"></td>';
    ...
    addChildren: function ($node, data) {
      ...
      if (!$node.find(".symbol").length) {
        $node
          .children(".title")
          .prepend(
            '<i class="oci ' + this.options.parentNodeSymbol + ' symbol"></i>'
          );
      }
      if (this.isInAction($node)) {
        this.switchVerticalArrow($node.children(".bottomEdge"));
    ...
    buildParentNode: function ($currentRoot, data) {
      ...
      var $table = $("<table>")
        .append(
          $("<tr>").append($('<td colspan="2">').append(this.createNode(data)))
        )
        .append(
          '<tr class="lines"><td colspan="2"><div class="downLine"></div></td></tr>'
        )
    ...
    addParent: function ($currentRoot, data) {
      ...
      if (!$currentRoot.children(".topEdge").length) {
        $currentRoot
          .children(".title")
          .after('<i class="edge verticalEdge topEdge oci"></i>');
      }
    ...
    buildSiblingNode: function ($nodeChart, data) {
      var newSiblingCount = $.isArray(data)
        ? data.length
        : data.children.length;
      var existingSibligCount = $nodeChart.parent().is("td")
        ? $nodeChart.closest("tr").children().length
        : 1;
      ...
      if ($nodeChart.parent().is("td")) {
        ...
        if (existingSibligCount > 1) {
          this.complementLine(
            $siblingTds
              .eq(0)
              .before($nodeChart.closest("td").siblings().addBack().unwrap()),
            siblingCount,
            existingSibligCount
          );
        } else {
          this.complementLine(
            $siblingTds
              .eq(insertPostion)
              .after($nodeChart.closest("td").unwrap()),
            siblingCount,
            1
          );
        }
        ...
        this.complementLine(
          $nodeChart
            ...
            .children()
            .eq(insertPostion)
            .after($('<td colspan="2">').append($nodeChart)),
          siblingCount,
          1
        );
      }
      ...
      if ($parent.is("td")) {
        if (this.getNodeState($node, "siblings").exist) {
          ...
          $sibs
            .slice(0, 2)
            .children()
            .attr("colspan", $sibs.eq(2).children().length);
          $parent.remove();
        } else {
          $sibs
            .eq(0)
    ...
    hideDropZones: function () {
      ...
      orgChartObj.$chart.find(".allowedDrop").removeClass("allowedDrop");
    },
    //
    showDropZones: function (dragged) {
      ...
      var orgChartObj = this;
      orgChartObj.$chart.find(".node").each(function (index, node) {
        $(node).addClass("allowedDrop");
      });
    ...
    export: function (exportFilename, exportFileextension) {
      ...
      if (!$mask.length) {
        $chartContainer.append(
          '<div class="mask"><i class="oci oci-spinner spinner"></i></div>'
        );
      } else {
