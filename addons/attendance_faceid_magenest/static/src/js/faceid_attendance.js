odoo.define("faceid_attendance_odoo.faceid_attendance", function (require) {
  ...
  var MyAttendances2 = AbstractAction.extend({
    ...
    events: {
      "click .o_form_binary_file_web_cam": _.debounce(
        function () {
          this.webcam_attendance();
        },
        200,
        true
      ),
    ...
    willStart: function () {
      ...
      var def = this._rpc({
        ...
        args: [
          ...
          ["attendance_state", "name", "hours_today", "user_id"],
        ],
      }).then(function (res) {
        self.employee = res.length && res[0];
        if (res.length) {
          self.hours_today = field_utils.format.float_time(
  ...
  core.action_registry.add(
    "faceid_attendance_odoo_my_attendance",
    MyAttendances2
  );
