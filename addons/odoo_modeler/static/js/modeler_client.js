...
odoo.define("funenc.ModelerProjectClient", function (require) {
  ...
  var ModelerClient = AbstractAction.extend({
    ...
    start: function () {
      ...
      // 移除connect
      core.bus.on(
        "funec_modeler_remove_dest_con",
        this,
        this.remove_dest_connection
      );
      core.bus.on(
        "funec_modeler_remove_dest_table_con",
        this,
        this.remove_dest_table_connection
      );
    },
    ...
    get_name_space: function () {
      return this.project_info.name_space
        ? this.project_info.name_space
        : "funenc";
    },
    ...
    init_tables: function () {
      ...
      for (var i = 0; i < this.project_info.tables.length; i++) {
        ...
        defs.push(tmp_table.appendTo(self.model_container));
        self.tables.push(tmp_table);
      }

      var self = this;
      Promise.all(defs).then(function (resultIds) {
    ...
    deal_new_table: function (table_id) {
      ...
      this._rpc({
        ...
        var tmp_table = new ModelerTable(self, table);
        tmp_table.appendTo(self.model_container).then(function () {
          self.tables.push(tmp_table);
