  ...
  var ModelerTable = Widget.extend({
        ...
        },
        {
          on_close: function (res) {
            ...
            if (res.data) {
              ...
              self.modeler_instance.revalidate(self.$el);
            }
          },
        }
      );
    ...
    _add_new_field: function (field_id) {
      ...
      this._rpc({
        ...
        tmp.appendTo(self.$field_container);
        self.fields.push(tmp);
        if (field.comodel_id && field.comodel_id[0] != self.table_info.id) {
          // 建立建接
    ...
    set_connection: function (field, field_con) {
      if (
        field.field_info.field_type != "Many2one" &&
    ...
    download_table_files: function () {
      ...
      var blob = new Blob([view_xml, csv], {
      ...
      this._rpc({
        ...
        window.cefQuery({
          ...
          onSuccess: function (response) {
            self.do_warn("tip", "save success");
          },
          onFailure: function (error_code, error_message) {
            self.do_warn("tip", "save failed");
          },
        });
      });
    },
    ...
    _on_del_table: function (event) {
      ...
      if (this.table_info.project_id[0] != this.project_info.id) {
        this.do_warn("warning", "do not del data of outer project!");
        return;
      }
