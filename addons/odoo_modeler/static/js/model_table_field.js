  ...
  var TableField = Widget.extend({
    ...
    _update_field_info: function () {
      ...
      this._rpc({
        ...
        self
          .$(".field_name")
          .text(self.field_info.name + "(" + self.field_info.cn_name + ")");
        // 检查类型是否发生改变，如果发生改变需要重新关联
        var need_reconnect = false;
        // 关联类型发生变化
        if (
          self._is_relation_field(old_field_info) !=
          self._is_relation_field(self.field_info)
        ) {
          need_reconnect = true;
        } else if (old_field_info.name != self.field_info.name) {
          // 名称发生变化
          need_reconnect = true;
        }
        // 通知table重新联接
      ...
      if (this.field_info.related) {
        ...
        switch (this.field_info.field_type) {
          case "Many2many":
            ...
            attrs.push(
              "comodel_name='" +
                this.field_info.com_project_info.name +
                "." +
                this.field_info.comodel_id[1] +
                "'"
            );
          ...
          case "One2many":
            ...
            attrs.push(
              "inverse_name='" + this.field_info.inverse_name[1] + "'"
            );
            break;
          case "Many2one":
            if (!this.field_info.com_project_info) {
