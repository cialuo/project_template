  ...
  return (
    <div onDrop={onDrop} onDragOver={onDragOver}>
      ...
      <div onClick={toggle}>{employee_show}</div>
        ...
        <OverlayPanel ref={el => op.current = el} showCloseIcon  id="overlay_panel" style={{ width: '30%', marginLeft: '-7%' }}>
...
const calculateExpandedKeys = (search, data, vde) => {
  if (search.length === 0 || !data) {
    ...
    return Object.keys(vde).reduce((prev, cur) => {
      ...
      const firstPath = item ? item.path.split(",") : null;
      if (firstPath) {
...
const View = (props) => {
  ...
  return (
    <div className="p-grid p-dir-col" style={{marginTop: '2vh', marginTop: '1vh', backgroundColor: '#f0eeee'}}>
      <div className="p-col">
        <button onClick={add_department({ is_ban: false })} className="btn btn-primary o_list_button_add">
          Add department
        </button>{" "}
        &nbsp;
        <button onClick={add_department({ is_ban: true })} className="btn btn-primary o_list_button_add">
