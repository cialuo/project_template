odoo.define("employee_time_table.time_working_table", function (require) {
  ...
  var EmployeeTimeTable = AbstractAction.extend({
    ...
    render: function () {
      ...
      //        });
      // return org_chart;
      var org_chart = QWeb.render("employee_time_table.time_working_template", {
      ...
      if (org_chart) {
        this.$el.find(".icon-leave").popover({
          content: function (e) {
            ...
            var leave_data = [];
            var current_time_interval = parseInt(
              $(this).attr("data-time-interval")
            );
                  ...
                  if (field[2] == leave_id) {
                    for (var i = 0; i < field[3].length; i++) {
                      ...
                      for (var j = 0; j < leave_data.length; j++) {
                        if (
                          field[3][i]["reason"] == leave_data[j]["reason"] &&
                          field[3][i]["leave_from"] ==
                            leave_data[j]["leave_from"] &&
                          field[3][i]["leave_to"] == leave_data[j]["leave_to"]
                        ) {
                          added = true;
                        }
                      }
                      if (!added) {
                        if (current_time_interval == 0) {
                          if (
                            parseInt(
                              field[3][i]["leave_from"].substring(11, 13)
                            ) < 12
                          ) {
                          ...
                          if (
                            parseInt(
                              field[3][i]["leave_to"].substring(11, 13)
                            ) >= 12
                          ) {
                            leave_data.push(field[3][i]);
                          }
            ...
            return QWeb.render("employee_time_table.leave", {
              value: leave_data,
            });
          },
          html: true,
        ...
        this.$el.find(".icon-meeting").popover({
          content: function (e) {
                  ...
                  if (fieldx[2] == meeting_id) {
                    _.each(fieldx[3], function (list) {
                      ...
                      for (var i = 0; i < meeting_data.length; i++) {
                        if (
                          list[3][0] == meeting_data[i][0] &&
                          list[3][1] == meeting_data[i][1] &&
                          list[3][2] == meeting_data[i][2]
                        ) {
                          added = true;
                        }
                      }
            ...
            return QWeb.render("employee_time_table.meeting", {
              value: meeting_data,
            });
          },
          html: true,
        ...
        this.$el.find(".timesheet").popover({
          content: function (e) {
                ...
                _.each(datai.timesheet, function (fieldi) {
                  if (fieldi[0] == employee_id) {
                    ...
                    if (total_timesheet_tmp) {
                      total_timesheet_tmp =
                        Math.round(total_timesheet_tmp * 100) / 100;
                    }
                    if (week_timesheet_tmp) {
                      week_timesheet_tmp =
                        Math.round(week_timesheet_tmp * 100) / 100;
                    }
                    if (today_timesheet_tmp) {
                      today_timesheet_tmp =
                        Math.round(today_timesheet_tmp * 100) / 100;
                    }
                    timesheet_data.push(
                      total_timesheet_tmp,
                      week_timesheet_tmp,
            ...
            var timesheet_detail = QWeb.render(
              "employee_time_table.timesheet",
              { value: timesheet_data }
            );
        ...
        this.$el.find(".flag-mon").popover({
          content: function (e) {
            ...
            return QWeb.render("attendance.day.off.description", {
              value: dayoff_data,
            });
          },
          html: true,
    ...
    _onSearchEmp: function (event) {
      ...
      _.each(all, function (element) {
        ...
        var current_element_name = $(element)
          .attr("data-name")
          .toLowerCase()
          .trim();
