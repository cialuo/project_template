        ...
        var X2Many2dMatrixRenderer = BasicRenderer.extend({
            ...
            _renderHeaderCell: function (node) {
                ...
                if (node.attrs.widget) {
                    description = this.state.fieldsInfo.list[name].Widget
            ...
            _renderRow: function (row) {
                ...
                var $cells = _.map(
                    ...
                    function (column, index) {
                        ...
                        return this._renderBodyCell(record, column, index, {
                            mode: "",
                        });
                    }.bind(this)
                );
            ...
            _renderBodyCell: function (record, node, colIndex, options) {
                ...
                if (node.tag === "field") {
                    var typeClass =
                        FIELD_CLASSES[this.state.fields[node.attrs.name].type];
                    if (typeClass) {
                ...
                if (
                    modifiers.invisible &&
                    !(options && options.renderInvisible)
                ) {
                    return $td;
                }
            ...
            _onNavigationMove: function (event) {
                ...
                if (move) {
                    ...
                    index = this.allFieldWidgets[target.record.id].indexOf(
                        target
                    );
            ...
            confirmUpdate: function (state, id, fields, ev) {
                var self = this;
                this.state = state;
                return this.confirmChange(state, id, fields, ev).then(
                    function () {
                        self._refresh(id);
                    }
                );
