    ...
    $.fn.resizableColumns = function () {
        ...
        _thead.find("th").each(function () {
            ...
            if ($(this).is(":not(:last-child)"))
                $(this).append(
                    "<div class='resizer' style='position:absolute;top:0px;right:-3px;bottom:0px;width:6px;z-index:999;background:transparent;cursor:col-resize'></div>"
                );
        });
        ...
        _table.find(".resizer").mousedown(function (e) {
            ...
            $(_thead)
                .find(
                    "tr:first-child th:nth-child(" +
                        ($(this).closest("th").index() + 1) +
                        ") .resizer"
                )
                .closest("th")
                .addClass("resizing");
        ...
        _table.mousemove(function (e) {
            if (isColResizing) {
                ...
                if (_resizing.length == 1) {
                    ...
                    var _setWidth =
                        _resizing.closest("th").innerWidth() + _widthDiff;
