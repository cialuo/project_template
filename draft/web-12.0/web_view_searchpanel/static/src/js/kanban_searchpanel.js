...
odoo.define("web_view_searchpanel.SearchPanel", function (require) {
    ...
    var SearchPanel = Widget.extend({
        ...
        events: {
            "click .o_search_panel_category_value header":
                "_onCategoryValueClicked",
            "click .o_search_panel_category_value .o_toggle_fold":
                "_onToggleFoldCategory",
            "click .o_search_panel_filter_group .o_toggle_fold":
                "_onToggleFoldFilterGroup",
            "change .o_search_panel_filter_value > div > input":
                "_onFilterValueChanged",
            "change .o_search_panel_filter_group > div > input":
                "_onFilterGroupChanged",
        },
        ...
        update: function (params) {
            if (this.loadProm.state() === "resolved") {
                ...
                if (
                    this.needReload ||
                    currentSearchDomainStr !== newSearchDomainStr
                ) {
        ...
        _createCategoryTree: function (categoryId, values) {
            ...
            values.forEach(function (value) {
                category.values[value.id] = _.extend({}, value, {
                    ...
                    folded: true,
                    parentId:
                        (value[parentField] && value[parentField][0]) || false,
                });
            });
            ...
            category.rootIds = Object.keys(category.values).filter(function (
                valueId
            ) {
                var value = category.values[valueId];
            ...
            if (!this.loadPromLazy) {
                // set active value
                var validValues = _.pluck(category.values, "id").concat([
                    false,
                ]);
                ...
                category.activeValueId = _.contains(validValues, value)
                    ? value
                    : false;

                // unfold ancestor values of active value to make it is visible
                if (category.activeValueId) {
                    var parentValueIds = this._getAncestorValueIds(
                        category,
            ...
            if (filter.groupBy) {
                ...
                values.forEach(function (value) {
                    ...
                    if (!groups[groupId]) {
                        ...
                        groups[groupId].folded =
                            (oldGroup && oldGroup.folded) || false;
                    }
                ...
                Object.keys(filter.groups).forEach(function (groupId) {
                    filter.values = _.extend(
                        filter.values,
                        filter.groups[groupId].values
                    );
                });
            } else {
                values.forEach(function (value) {
        ...
        _getFilterDomain: function () {
            ...
            function getCheckedValueIds(values) {
                return Object.keys(values).reduce(function (
                    checkedValues,
                    valueId
                ) {
        ...
        _getAncestorValueIds: function (category, categoryValueId) {
            ...
            if (parentId) {
                return [parentId].concat(
                    this._getAncestorValueIds(category, parentId)
                );
            }
            return [];
        },
        ...
        _render: function () {
            ...
            var categories = Object.keys(this.categories).map(function (
                categoryId
            ) {
        ...
        _renderFilter: function (filter) {
            var $filter = $(
                qweb.render("SearchPanel.Filter", { filter: filter })
            );
            ...
            Object.keys(filter.groups || {}).forEach(function (groupId) {
                ...
                if (groupId !== "false" && state === "indeterminate") {
                    $filter
                        .find(
                            ".o_search_panel_filter_group[data-group-id=" +
                                groupId +
                                "] input"
                        )
                        .get(0).indeterminate = true;
                }
            });
        ...
        _onCategoryValueClicked: function (ev) {
            ev.stopPropagation();
            var $item = $(ev.currentTarget).closest(
        ...
        _onFilterValueChanged: function (ev) {
            ...
            if (group) {
                var valuePartition = _.partition(
    ...
    if (config.device.isMobile) {
        SearchPanel.include({
            ...
            _getCategorySelection: function () {
                var self = this;
                return Object.keys(this.categories).reduce(function (
                    selection,
                    categoryId
                ) {
                    var category = self.categories[categoryId];
                    if (category.activeValueId) {
                        var ancestorIds = [category.activeValueId].concat(
                            self._getAncestorValueIds(
                                category,
                                category.activeValueId
                            )
                        );
            ...
            _getFilterSelection: function () {
                var self = this;
                return Object.keys(this.filters).reduce(function (
                    selection,
                    filterId
                ) {
                    var filter = self.filters[filterId];
                    if (filter.groups) {
                        Object.keys(filter.groups).forEach(function (groupId) {
                            var group = filter.groups[groupId];
                            Object.keys(group.values).forEach(function (
                                valueId
                            ) {
            ...
            _render: function () {
                this._super.apply(this, arguments);
                this.$el.prepend(
                    qweb.render("SearchPanel.MobileSummary", {
