    ...
    var Webcam = {
            ...
            while (offset < binFile.byteLength) {
                ...
                if (marker == 225) {
                    ...
                    for (var i = 0; i < entries; i++) {
                        ...
                        if (
                            dataView.getUint16(entryOffset, !bigEnd) == 0x0112
                        ) {
                            var valueType = dataView.getUint16(
                                entryOffset + 2,
                                !bigEnd
                            );
                            var numValues = dataView.getUint32(
                                entryOffset + 4,
                                !bigEnd
                            );
        ...
        attach: function (elem) {
                    ...
                    function (event) {
                        if (
                            ...
                            fileReader.addEventListener(
                                "load",
                                function (e) {
                                    var orientation = self.exifOrientation(
                                        e.target.result
                                    );
                                    if (orientation > 1) {
            ...
            if (this.params.crop_width && this.params.crop_height) {
                var scaled_crop_width = Math.floor(
                    this.params.crop_width * scaleX
                );
                var scaled_crop_height = Math.floor(
                    this.params.crop_height * scaleY
                );
        ...
        dispatch: function () {
            ...
            if (this.hooks[name] && this.hooks[name].length) {
                for (
                    var idx = 0, len = this.hooks[name].length;
                    idx < len;
                    idx++
                ) {
        ...
        snap: function (user_callback, user_canvas) {
            ...
            var func = function () {
                ...
                if (params.crop_width && params.crop_height) {
                    ...
                    crop_context.drawImage(
                        canvas,
                        Math.floor(
                            params.dest_width / 2 - params.crop_width / 2
                        ),
                        Math.floor(
                            params.dest_height / 2 - params.crop_height / 2
                        ),
                        params.crop_width,
                        params.crop_height,
                        0,
            ...
            if (this.userMedia) {
                ...
                var div = document.getElementById(
                    this.container.id + "-ios_div"
                );
                var img = document.getElementById(
                    this.container.id + "-ios_img"
                );
                var input = document.getElementById(
                    this.container.id + "-ios_input"
                );
        ...
        base64DecToArr: function (sBase64, nBlocksSize) {
            ...
            for (
                ...
                nMod4 = nInIdx & 3;
                nUint24 |=
                    this.b64ToUint6(sB64Enc.charCodeAt(nInIdx)) <<
                    (18 - 6 * nMod4);
