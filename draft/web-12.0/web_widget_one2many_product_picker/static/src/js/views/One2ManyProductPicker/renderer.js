        ...
        var One2ManyProductPickerRenderer = BasicRenderer.extend({
                ...
                for (var index_destroy in to_destroy) {
                    ...
                    for (var eb = this.widgets.length - 1; eb >= 0; --eb) {
                        ...
                        if (
                            widget &&
                            widget.state &&
                            widget.state.data[this.options.field_map.product]
                                .data.id === widget_product_id
                        ) {
                            found = true;
                            break;
                        }
            ...
            _updateStateRecords: function (old_states) {
                ...
                for (var index in old_states) {
                    ...
                    for (var e in this.state.data) {
                        ...
                        if (
                            current_state.id === old_state.id ||
                            (typeof current_state.data.id !== "undefined" &&
                                current_state.data.id === old_state.data.id)
                        ) {
                            found = true;
                            break;
                        }
                    ...
                    for (var e = this.widgets.length - 1; e >= 0; --e) {
                        ...
                        if (
                            ...
                            (typeof state.data.id !== "undefined" &&
                                widget.state.data.id === state.data.id)
                        ) {
            ...
            _renderView: function () {
                ...
                return $.Deferred(function (d) {
                    ...
                    defs[0].then(function () {
                        ...
                        self.showLoadMore(
                            self.last_search_data_count >=
                                self.options.records_per_page
                        );
                        if (self._isInDom) {
            ...
            _appendSearchRecords: function (
                ...
                _.each(processed_records, function (search_record) {
                    var state_data = self._getRecordDataById(
                        search_record.__id
                    );
                    ...
                    var def = ProductPickerRecord.appendTo(
                        self.$recordsContainer
                    ).then(
                ...
                return [
                    $.when.apply($, defs).then(function () {
                        if (!no_attach_widgets && self._isInDom) {
                            var new_widgets = self.widgets.slice(
                                cur_widget_index
                            );
            ...
            _onRecordFlip: function (evt) {
                ...
                if (typeof prev_widget_index !== "undefined") {
                    ...
                    for (var index in this.widgets[prev_widget_index].widgets
                        .back) {
                        var widget = this.widgets[prev_widget_index].widgets
                            .back[index];
