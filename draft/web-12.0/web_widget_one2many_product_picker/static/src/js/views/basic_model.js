...
odoo.define(
    "web_widget_one2many_product_picker.BasicModel",
    function (require) {
        "use strict";
        ...
        BasicModel.include({
            ...
            createVirtualRecord: function (listID, options) {
                ...
                return $.Deferred(function (d) {
                    self._makeDefaultRecord(list.model, params).then(function (
                        recordID
                    ) {
            ...
            _applyChangeNoWarnings: function (recordID, changes, options) {
                ...
                this._visitChildren(record, function (elem) {
                    initialData[elem.id] = $.extend(
                        true,
                        {},
                        _.pick(elem, "data", "_changes")
                    );
                });
                ...
                return $.when.apply($, defs).then(function () {
                    ...
                    if (onChangeFields.length) {
                        self._performOnChangeNoWarnings(
                            record,
                            onChangeFields,
                            options.viewType
                        )
                    ...
                    return onchangeDef.then(function (fieldNames) {
                        ...
                        return self
                            ._fetchSpecialData(record)
                            .then(function (fieldNames2) {
            ...
            _performOnChangeNoWarnings: function (record, fields, viewType) {
                ...
                return self
                    ...
                    .then(function (result) {
                        ...
                        if (result.domain) {
                            record._domains = _.extend(
                                record._domains,
                                result.domain
                            );
                        }
                        return self
                            ._applyOnChange(result.value, record)
                            .then(function () {
                                return result;
                            });
                    });
