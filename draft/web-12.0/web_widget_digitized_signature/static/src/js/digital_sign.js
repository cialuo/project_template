odoo.define(
    "web_widget_digitized_signature.web_digital_sign",
    function (require) {
        "use strict";
        ...
        var FieldSignature = BasicFields.FieldBinaryImage.extend({
            ...
            events: _.extend(
                {},
                BasicFields.FieldBinaryImage.prototype.events,
                {
            ...
            start: function () {
                ...
                this.empty_sign = this.$(".signature").jSignature(
                    "getData",
                    "image"
                );
                self._render();
            ...
            _on_save_sign: function (value_) {
                var self = this;
                this.$("> img").remove();
                var signature = this.$(".signature").jSignature(
                    "getData",
                    "image"
                );
            ...
            _render: function () {
                ...
                if (this.mode === "readonly") {
                    var $img = $(
                        QWeb.render("FieldBinaryImage-img", {
                            widget: self,
                            url: url,
                        })
                    );
                    ...
                    if (this.value) {
                        ...
                        self._rpc({
                            ...
                            if (data) {
                                var field_desc = _.values(
                                    _.pick(data[0], field_name)
                                );
