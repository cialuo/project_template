    ...
    function getColors($e) {
        ...
        if (frontcolorcomponents && backcolorcomponents) {
            ...
            frontcolorbrightness = Math.max.apply(null, [
                backcolorcomponents.r,
                backcolorcomponents.g,
                backcolorcomponents.b,
            ]);
            adjusted = Math.round(
                frontcolorbrightness +
                    -1 * (frontcolorbrightness - backcolorbrightness) * 0.75
            ); // "dimming" the difference between pen and back.
        ...
        return {
            color: frontcolor,
            "background-color": backcolorcomponents
                ? toRGBfn(backcolorcomponents)
                : backcolor,
        ...
        this.resizeTo = function (length) {
            ...
            if (this.x === 0 && this.y === 0) {
                ...
                var proportion = Math.abs(this.y / this.x),
                    x = Math.sqrt(
                        Math.pow(length, 2) / (1 + Math.pow(proportion, 2))
                    ),
                    y = proportion * x;
                this._length = length;
            ...
            if (divisor === 0) {
                ...
                return (
                    Math.acos(
                        Math.min(
                            Math.max(
                                (this.x * vectorB.x + this.y * vectorB.y) /
                                    divisor,
                                -1.0
                            ),
                            1.0
    ...
    function DataEngine(
        ...
        this.addToStroke = function (point) {
            if (
                ...
                Math.abs(point.x - this._lastPoint.x) +
                    Math.abs(point.y - this._lastPoint.y) >
                    4
            ) {
    ...
    var basicDot = function (ctx, x, y, size) {
        ...
        strokeAddCallback = function (stroke, positionInStroke) {
            ...
            var Cpoint = new Point(
                    stroke.x[positionInStroke - 1],
                    stroke.y[positionInStroke - 1]
                ),
                Dpoint = new Point(
                    stroke.x[positionInStroke],
                    stroke.y[positionInStroke]
                ),
            ...
            if (positionInStroke > 1) {
                ...
                if (BCvector.getLength() > this.lineCurveThreshold) {
                    ...
                    if (positionInStroke > 2) {
                        ...
                        ABvector = new Point(
                            stroke.x[positionInStroke - 3],
                            stroke.y[positionInStroke - 3]
                    ...
                    var minlenfraction = 0.05,
                        ...
                        BCP1vector = new Vector(
                            ABvector.x + BCvector.x,
                            ABvector.y + BCvector.y
                        ).resizeTo(Math.max(minlenfraction, ABCangle) * maxlen),
                        CCP2vector = new Vector(
            ...
            if (positionInStroke > 0) {
                ...
                if (BCvector.getLength() > this.lineCurveThreshold) {
                    ...
                    if (positionInStroke > 1) {
                        ...
                        var BCP1vector = new Vector(
                            ABvector.x + BCvector.x,
                            ABvector.y + BCvector.y
                        ).resizeTo(BCvector.getLength() / 2);
                        basicCurve(
                            this.canvasContext,
                            Bpoint.x,
                            Bpoint.y,
...
*/
    ...
    function conditionallyLinkCanvasResizeToWindowResize(
        jSignatureInstance,
        settingsWidth,
        apinamespace,
        globalEvents
        ...
        if (
            settingsWidth === "ratio" ||
            settingsWidth.split("")[settingsWidth.length - 1] === "%"
        ) {
            this.eventTokens[
    ...
    function jSignatureClass(parent, options, instanceExtensions) {
        ...
        this.$controlbarUpper = (function () {
            ...
            return $('<div style="' + controlbarstyle + '"></div>').appendTo(
                $parent
            );
        })();
        ...
        settings.lineWidth = (function (defaultLineWidth, canvasWidth) {
            if (!defaultLineWidth) {
                return Math.max(
                    Math.round(
                        canvasWidth / 400
                    ) /*+1 pixel for every extra 300px of width.*/,
            ...
            var shiftX,
                ...
                getPointFromEvent = function (e) {
                    ...
                    return new Point(
                        ...
                        Math.round(firstEvent.pageY + shiftY) +
    ...
    jSignatureClass.prototype.initializeCanvas = function (settings) {
        ...
        $canvas
            ...
            .css(
                "height",
                settings.height === "ratio" || !settings.height
                    ? 1
                    : settings.height.toString(10)
            )
            .css(
                "width",
                settings.width === "ratio" || !settings.width
                    ? 1
                    : settings.width.toString(10)
            );
    ...
    var GlobalJSignatureObjectInitializer = function (window) {
        ...
        var jSignatureInstanceExtensions = {
            exampleExtension: function (extensionName) {
                ...
                this.events.subscribe(
                    ...
                    function () {
                        if (
                            this.settings.hasOwnProperty(
                                "non-existent setting category?"
                            )
                        ) {
        ...
        function _renderImageOnCanvas(data, formattype, rerendercallable) {
            ...
            c.onload = function () {
                ...
                a.drawImage(
                    ...
                    0,
                    0,
                    c.width < e.width ? c.width : e.width,
                    c.height < e.height ? c.height : e.height
                );
                a.shadowColor = b;
            };
        ...
        function _setDrawingData(data, formattype) {
            ...
            var $canvas = this.find("canvas." + apinamespace).add(
                this.filter("canvas." + apinamespace)
            );

            if (!importplugins.hasOwnProperty(formattype)) {
                throw new Error(
                    apinamespace +
                        " is unable to find import plugin with for format '" +
                        String(formattype) +
                        "'"
                );
                ...
                importplugins[formattype].call(
                    ...
                    (function (jSignatureInstance) {
                        return function () {
                            return jSignatureInstance.resetCanvas.apply(
                                jSignatureInstance,
                                arguments
                            );
                        };
        ...
        var plugins = {
                export: exportplugins,
                import: importplugins,
                instance: jSignatureInstanceExtensions,
            },
            methods = {
                ...
                getData: function (formattype) {
                    var undef,
                        $canvas = this.find("canvas." + apinamespace).add(
                            this.filter("canvas." + apinamespace)
                        );
                    if (formattype === undef) formattype = "default";
                    if (
                        $canvas.length !== 0 &&
                        exportplugins.hasOwnProperty(formattype)
                    ) {
