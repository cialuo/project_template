...
odoo.define("web_pivot_computed_measure.PivotModel", function (require) {
    ...
    PivotModel.include({
         ...
         * @param {string} format
         */
        createComputedMeasure: function (
            id,
            field1,
            field2,
            operation,
            ...
            var oper_human = operation
                .replace(
                    /m1/g,
                    fieldM1.__computed_id
                        ? "(" + fieldM1.string + ")"
                        : fieldM1.string
                )
                .replace(
                    /m2/g,
                    fieldM2.__computed_id
                        ? "(" + fieldM2.string + ")"
                        : fieldM2.string
                );
            ...
            return this._createVirtualMeasure(
                this._computed_measures[cmTotal - 1]
            );
        },
        ...
        _fillComputedMeasuresData: function (dataPoint, dataPointComp) {
            ...
            _.each(this._computed_measures, function (cm) {
                ...
                if (dataPointComp) {
                    ...
                    var resComparison = py.eval(
                        cm.operation,
                        dataPointComp.comparison
                    );
                    dataPoint[cm.id] = {
                        data: resData,
                ...
                if (res.length) {
                    ...
                    for (var k = 0; k < len2; ++k) {
                        ...
                        if ("__comparisonDomain" in dataPoint) {
                            ...
                            this._fillComputedMeasuresData(
                                dataPointComp.comparison
                            );
        ...
        toggleMeasure: function (field) {
            if (this._isMeasureEnabled(field)) {
                ...
                if (
                    umeasures.length &&
                    this._isMeasureEnabled(umeasures[0].id)
                ) {
                    return $.Deferred(function (d) {
                        d.reject(
                            _t(
