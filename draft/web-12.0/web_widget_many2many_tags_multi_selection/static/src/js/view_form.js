odoo.define(
    "web_widget_many2many_tags_multi_selection.multiple_tags",
    function (require) {
        "use strict";
                ...
                return new dialogs.SelectCreateDialog(
                    ...
                    _.extend({}, self.nodeOptions, {
                        ...
                        title:
                            (view === "search"
                                ? _t("Search: ")
                                : _t("Create: ")) + self.string,
                        initial_ids: ids
                            ? _.map(ids, function (x) {
                                  return x[0];
                              })
                            : undefined,
                        initial_view: view,
                        disable_multiple_selection:
                            self.field.type !== "many2many",
        ...
        rel_fields.FieldMany2ManyTags.include({
            ...
            _renderEdit: function () {
                ...
                return this._super.apply(this, arguments).then(function () {
                    if (need_modal) {
                        self.many2one._searchCreatePopup(
                            "form",
                            false,
                            self.many2one._createContext("")
                        );
                    }
                });
            },
